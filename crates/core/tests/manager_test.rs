...
use graph_tally_graph::{Receipt, ReceiptAggregateVoucher, SignedReceipt};
...
async fn test_retryable_checks(
    ...
    assert_eq!(
        ...
        graph_tally_core::Error::ReceiptError(
            graph_tally_core::receipt::ReceiptError::RetryableCheck("Retryable error".to_string())
        )
        .to_string()
    );
}
